...
import { EditProductSupplierComponent } from 'src/app/components/edit-product-supplier/edit-product-supplier.component';
...
export class SupplierPage implements OnInit {
  ...
  title:string = 'Product Suppliers';
  allData = [];
  tableData = [];
  ...
  all_columns:any[] = [
    ...
    {prop: 'markup', name: 'Markup(%)', sortable: true, checked: true},
  ];
  ...
  async openEdit(row) {
    const popover = await this.popoverController.create({
      component: EditProductSupplierComponent,
      //event: ev,
  ...
  delete(row:any) {
    this.alertService.presentAlertConfirm('Confirm Delete?', 'Are you sure to want to delete this supplier?', () => {
      this.utilService.delete('product/supplier?_id=' + row._id).subscribe(result => {
        this.initTable();
      }, async error => {
  ...
  async openSearch() {
    const popover = await this.popoverController.create({
      ...
      translucent: true,
      componentProps: {keyword: this.filter.keyword, title: 'Supplier', label: 'Name / Description'}
    });
