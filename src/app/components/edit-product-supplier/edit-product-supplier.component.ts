...
@Component({
  ...
  styleUrls: ['./edit-product-supplier.component.scss'],
})
export class EditProductSupplierComponent implements OnInit {
  ...
  ngOnInit() {
    if(this.row._id) {
      ...
      this.form.get('markup').setValue(this.row.markup);
    }
  }
  ...
  async submit() {
    ...
    if(this.row._id) {
      ...
      this.utilService.put('product/supplier', data).subscribe(async result => {
        await this.final_process(result);
      })
    } else {     
      data.private_web_address = this.row.private_web_address; 
      this.utilService.post('product/supplier', data).subscribe(async result => {
        await this.final_process(result);
      })
    }
  }
  ...
  get nameInputError(): string {
    ...
    if (this.dup_error) {return 'Already existing supplier.';}
  }
}
