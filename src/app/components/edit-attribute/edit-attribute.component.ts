...
export class EditAttributeComponent implements OnInit {
  ...
  title:string = 'Add New Attribute';  
  row:any;
  form: FormGroup;
  ...
  async submit() {
    ...
    if(this.row._id) {
      ...
      this.utilService.put('product/attribute', data).subscribe(async result => {
        await this.final_process(result);
      })
    } else {     
      data.private_web_address = this.row.private_web_address; 
      this.utilService.post('product/attribute', data).subscribe(async result => {
        await this.final_process(result);
      })
    }
  }
  ...
  get nameInputError(): string {
    ...
    if (this.dup_error) {return 'Already existing attribute.';}
  }
}
